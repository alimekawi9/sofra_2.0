...
Note: `AnimatePresence` stays permanently mounted here — it detects exits by diffing the `children` it
receives across its own re-renders, so it must not itself be unmounted by the `!leaving` check; only the
`motion.div` inside it is conditional. The `Image` is sized `340×340` (not the source file's native
1254×1254) to match its actual max rendered CSS size (`width:min(78vw,340px)`), avoiding an oversized
`priority`-loaded request on this LCP-sensitive intro screen.
