# Kitchen Page Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Build `app/(chef)/kitchen/page.tsx` — a 'use client' page with two CRUD sections (Signatures and This Week's Pantry) backed by Supabase, scoped to the logged-in chef.

**Architecture:** Single client component; auth resolved on mount via `supabase.auth.getUser()`, uid stored in a ref for all subsequent queries. All 6 Supabase calls (2 selects, 2 inserts, 2 deletes) carry an explicit `.eq('chef_id', uid)` scope filter — reads are not trusted to RLS alone since SELECT policies are open to all authenticated users. Deletes use optimistic UI with snapshot-restore rollback.

**Tech Stack:** Next.js 14 App Router, Supabase JS client (`@supabase/ssr`), inline styles matching existing color palette.
...
|--------|------|----------------|
| Create | `app/(chef)/kitchen/page.tsx` | Full page — auth, data loading, Signatures CRUD, Pantry CRUD, styling |

No other files are touched.

---

### Task 1: Page shell — auth, layout, loading, error states

**Files:**
- Create: `app/(chef)/kitchen/page.tsx`
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginBottom: 16 }}>
                  <div style={{ display: 'flex', gap: 8 }}>
                    <input
                      ...
                      style={inputStyle}
                    />
                    <button onClick={() => void addSignature()} disabled={sigAdding} style={addBtnStyle(sigAdding)}>
                      {sigAdding ? '…' : 'Add'}
                    </button>
                  </div>
                  <input
              ...
              <section>
                ...
                <div style={{ display: 'flex', gap: 8, marginBottom: pantryAddError ? 8 : 16 }}>
                  <input
                    ...
                    style={inputStyle}
                  />
                  <button onClick={() => void addPantryItem()} disabled={pantryAdding} style={addBtnStyle(pantryAdding)}>
                    {pantryAdding ? '…' : 'Add'}
                  </button>
                </div>
                {pantryAddError && (
...
Run: `npx tsc --noEmit`
Expected: no output (exit 0). Fix any type errors before continuing.

- [ ] **Step 3: Manual security audit — verify all 6 queries**

Read `app/(chef)/kitchen/page.tsx` and confirm:

| # | Location | Filter present |
|---|----------|----------------|
| 1 | `loadData` → signatures SELECT | `.eq('chef_id', uid)` ✓ |
| 2 | `loadData` → pantry SELECT | `.eq('chef_id', uid)` ✓ |
| 3 | `addSignature` → INSERT payload | `chef_id: uid` ✓ |
| 4 | `addPantryItem` → INSERT payload | `chef_id: uid` ✓ |
| 5 | `deleteSignature` → DELETE | `.eq('chef_id', uid)` ✓ |
| 6 | `deletePantryItem` → DELETE | `.eq('chef_id', uid)` ✓ |

If any filter is missing, add it before committing.

- [ ] **Step 4: Manual rollback audit — verify delete failure path**

In `deleteSignature`:
1. `prev` captured **before** optimistic `setSignatures`
2. On `error`: `setSignatures(prev)` called (restores exact snapshot)
3. On `error`: `setSigDeleteError('...')` called (user sees message)

In `deletePantryItem`:
1. `prev` captured **before** optimistic `setPantry`
2. On `error`: `setPantry(prev)` called
3. On `error`: `setPantryDeleteError('...')` called

- [ ] **Step 5: Commit**

```bash
git add app/(chef)/kitchen/page.tsx
git commit -m "feat: add kitchen page with signatures and pantry CRUD"
```
